...
namespace DeusMod.Core.Loaders
{
    class ShaderLoader : IOrderedLoadable
    {
        ...
        public void Load()
        {
            ...
            var file = (TmodFile)info.Invoke(DeusMod.Instance, null);
        ...
        {
            var screenRef = new Ref<Effect>(DeusMod.Instance.Assets.Request<Effect>(path, ReLogic.Content.AssetRequestMode.ImmediateLoad).Value);
